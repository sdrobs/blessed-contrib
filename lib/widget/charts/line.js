...
//iteratee for lodash _.max
    ...
    function getMinY() {
      ...
      for(var i = 0; i < data.length; i++) {
        if (data[i].y.length)
          setMin[i] = _.min(data[i].y, getMin);
    ...
    var maxPadding = getMaxXLabelPadding(this.options.numYLabels, this.options.wholeNumbersOnly, this.options.abbreviate, getMinY())
    ...
    function getYPixel(val, minY) {
        var res = self.canvasSize.height - yPadding - (((self.canvasSize.height - yPadding) / (getMaxY()-getMinY())) * (val-getMinY()));
    ...
    var yLabelIncrement = (getMaxY()-getMinY())/this.options.numYLabels
    ...
    for(var i = minY; i < maxY; i += yLabelIncrement) {
        c.fillText(formatYLabel(i, maxY, minY, this.options.numYLabels, this.options.wholeNumbersOnly, this.options.abbreviate), xPadding - xLabelPadding, getYPixel(i, minY));
    }
